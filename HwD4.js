...
} else if (typeof y === "boolean" && y === false) {
  for (let i = 0; i < split.length; i++) {
    split[i] = split[i].slice(0, -1)
      }
      return split = split.join(' ');
} else return "y is not boolean";
}

console.log(deleteOne("Hello strive, Testing", true));
...
console.log(isThisAnEmail("mike@gmail.com"));
...
const dices = n => {
  let random = [];
  for(i = 0; i < n; i++) {
      random.push(Math.floor(Math.random() * n) + 1); 
    }
  return random = {
    sum: random.reduce((a, b) => a + b, 0),
    values: random
  }
  }
...
console.log(howManyDays(01, 2, 2021));
